...
void UndirectedGraphUserInterface::printUserOptions() {
    ...
    std::cout << "10. Print all the connected components of the graph\n";
    std::cout << "11. Get the minimum vertex cover (exponential - backtracking)\n";
    std::cout << "12. Get an approximate vertex cover (heuristic)\n";
    std::cout << "13. Exit\n";
}
...
void UndirectedGraphUserInterface::option10() {
    std::vector<UndirectedGraph> connectedComponents = graph.getConnectedComponents();
    for (auto& connectedComponent: connectedComponents) {
        std::cout << "==========================================\n";
        std::cout << "Vertices: ";
        auto iteratorPair = connectedComponent.parseSetOfVertices();
        for (auto it = iteratorPair.first; it != iteratorPair.second; ++it)
            std::cout << *it << " ";
        std::cout << '\n';
        std::cout << "Edges\n";
        for (auto it = iteratorPair.first; it != iteratorPair.second; ++it) {
            auto outboundIteratorPair = connectedComponent.parseNeighbours(*it);
            for (auto it2 = outboundIteratorPair.first; it2 != outboundIteratorPair.second; ++it2) {
                if (*it < *it2)
                    std::cout << *it << " " << *it2 << '\n';
            }
...
void UndirectedGraphUserInterface::option11() {
    auto vertexCover = graph.getMinimumVertexCover();
    std::cout << "Number of vertices in the vertex cover: " << vertexCover.size() << '\n';
    std::cout << "Vertex cover: ";
    for (const int& vertex: vertexCover) {
        std::cout << vertex << ' ';
    }
    std::cout << '\n';
}

void UndirectedGraphUserInterface::option12() {
    auto vertexCover = graph.getApproximateVertexCover();
    std::cout << "Number of vertices in the vertex cover: " << vertexCover.size() << '\n';
    std::cout << "Vertex cover: ";
    for (const int& vertex: vertexCover) {
        std::cout << vertex << ' ';
